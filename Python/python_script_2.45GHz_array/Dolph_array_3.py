...
substrate = x.create_box(origin=[-249.62,-sub_width/2,0],
                         sizes=[602.35,57.8,sub_thick],
                         name="subsstrate",
                         material="FR4_epoxy")
...
'''hfss.lumped_port(assignment="LP_rect",
...
elements = ["Patch_antenna","Ground_Plane"]
duplicate = hfss.modeler.duplicate_along_line(assignment=elements,
                                              vector=["-103.11mm","0mm","0mm"],
                                              clones=2)

duplicate_2 = hfss.modeler.duplicate_along_line(assignment=elements,
                                              vector=["103.11mm","0mm","0mm"],
                                              clones=3)
    
x_pos = [-2*103.11,-103.11,0,103.11,2*103.11,3*103.11]

for i, x_pos in enumerate(x_pos,start=1):
    
    x.create_rectangle(orientation="XZ",
                       origin=[-(feed_width/2)+x_pos,-sub_width/2,0],
                       sizes=[sub_thick,feed_width],
                       name=f"LP_rect_{i}")
    
    hfss.lumped_port(assignment=f"LP_rect_{i}",
                     reference="Ground_Plane",
                     integration_line=[[x_pos,-sub_width/2,0],
                                       [x_pos,-sub_width/2,sub_thick]],
                     impedance=50,
                     name=f"Lumped_port_{i}")


    

sources = {
    "Lumped_port_1:1": ("1.0W", "0deg"),      # Outer
    "Lumped_port_2:1": ("1.0086W", "0deg"),   # Middle
    "Lumped_port_3:1": ("1.4544W", "0deg"),   # Center
    "Lumped_port_4:1": ("1.4544W", "0deg"),   # Center
    "Lumped_port_5:1": ("1.0086W", "0deg"),   # Middle
    "Lumped_port_6:1": ("1.0W", "0deg")       # Outer
}
